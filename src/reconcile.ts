 ...
 */
const warningOnly: Array<{ key: string; reason: string }> = [
...
/**
 * Keys handled by `warningOnly` are valid Compose syntax we deliberately keep,
 * so unknown-key validation must never flag them. Derived from the table so a
 * new warn-only rule can't fall out of sync with `knownServiceKeys`.
 */
const warningOnlyKeys = new Set(warningOnly.map(({ key }) => key));

const stripKeys: Array<{ key: string; reason: string }> = [
  {
    key: "develop",
 ...
 * unknown-key findings are advisory only and never fail the deployment on their
...
const resourceTranslations: Array<{
  key: string;
  path: string[];
  target: string;
}> = [
...
class Diagnostics {
   ...
   * violation (fails the deployment when `strict-compatibility` is enabled).
   */
    ...
    if (strict && this.violations.length > 0) {
      throw new Error(
        `Compose specification contains ${this.violations.length} ` +
          ...
          this.violations.map((value) => `  - ${value}`).join("\n"),
      );
    }
  }
}
...
function translateResources(
  ...
  for (const { key, path, target } of resourceTranslations) {
    if (!(key in service)) {
      continue;
    }
...
function validateServiceKeys(
  ...
  for (const key of Object.keys(service)) {
    if (
      key.startsWith("x-") ||
      knownServiceKeys.has(key) ||
      warningOnlyKeys.has(key)
    ) {
      continue;
    }
    ...
    diagnostics.advise(
      `Unknown property "${key}" on service "${name}"` +
        suggestion(key, knownServiceKeys) +
        ".",
    );
  }
}
...
function validateTopLevelKeys(
  ...
  for (const key of Object.keys(spec)) {
    if (key.startsWith("x-") || knownTopLevelKeys.has(key)) {
      continue;
    }

    diagnostics.advise(
      `Unknown top-level property "${key}"` +
        suggestion(key, knownTopLevelKeys) +
        ".",
    );
  }
}
  ...
  if (Array.isArray(labels)) {
    ...
    for (const item of labels) {
      ...
      if (eq === -1) {
        ...
        map[label.slice(0, eq)] = label.slice(eq + 1);
      }
    }
...
function applyStrips(
  ...
  for (const { key, reason } of stripKeys) {
    if (key in service) {
      delete service[key];
...
function stripDroppedDependencies(
  ...
  for (const [name, service] of Object.entries(services)) {
    ...
    const kept = entry.depends_on.filter(
      (dependency) => !droppedSet.has(String(dependency)),
    );
...
function applyWarnOnly(
  ...
  for (const { key, reason } of warningOnly) {
    if (key in service) {
      diagnostics.advise(
